...
const ManageCourses = () => {
  ...
  return (
    ...
    >
      {/* Page Header */}
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8 pt-8 px-8">
        <div>
      ...
      {!isLoading && !isError && (
        <div className="hidden lg:block p-8 overflow-x-auto">
          <div className="min-w-[900px] bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden">
      ...
      {/* Mobile & Tablet Grid */}
      {!isLoading && !isError && (
        <div className="lg:hidden md:grid md:grid-cols-2 gap-4 p-4 sm:grid-cols-2 grid-cols-1">
          {courses.map((course, index) => (
            <motion.div
              key={course._id}
              initial={{ opacity: 0, y: 20 }}
              ...
              transition={{ delay: index * 0.05 }}
              whileHover={{ y: -2 }}
              className="bg-white rounded-lg shadow-sm border border-gray-100 p-4 transition-shadow"
            >
